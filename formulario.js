...
function Validacion() {
    ...
    if (FORM_dni == null || FORM_nombre == null || FORM_email == null
        ...
        return false;
    } else if (!expr.test(FORM_email)) { //Comprueba que el mail es v�lido
        alert("Error: La direccion de correo es incorrecta.");
        return false;
    } else if ((!(/^\d{8}[A-Z]$/.test(FORM_dni))) || (FORM_dni.charAt(8) != letras[(FORM_dni.substring(0, 8)) % 23])) { //Comprueba que el DNI es v�lido
